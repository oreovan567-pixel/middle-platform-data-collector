...
def kpi_summary():
    ...
    只查询 SLS overall 模块的同比/环比日活数据，
    配合前端已有的 module-usage 当期数据计算百分比。
    """
    start_date = request.args.get("start_date", "")
    end_date = request.args.get("end_date", "")
    ...
    def _query_overall(label, s_dt, e_dt):
        """查询一个时段的 overall 模块活跃人数"""
        ...
        import urllib.request
        # 只查 overall 模块（无 url 过滤）
        query = (
            f'* and host:"research-api.qimingdaren.com"'
            f' and not tianli_school_id:"-"'
            f' | SELECT COUNT(DISTINCT tianli_user_id) as count'
        )
        payload = {
            "queries": [{
                "refId": "overall",
        ...
        try:
            ...
            with urllib.request.urlopen(req, timeout=8) as resp:
                data = json.loads(resp.read().decode())
            frames = data.get("results", {}).get("overall", {}).get("frames", [])
            for frame in frames:
                values = frame.get("data", {}).get("values", [])
                if values and values[0]:
                    return int(values[0][0]) if values[0][0] else 0
            return 0
        except Exception as e:
            logger.warning("[%s] SLS overall 查询失败: %s", label, e)
            return 0
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=2) as pool:
        f_tb = pool.submit(_query_overall, "tb", tb_start, tb_end)
        f_hb = pool.submit(_query_overall, "hb", hb_start, hb_end)
        tb_daily = f_tb.result()
        hb_daily = f_hb.result()

    return jsonify({
        "tongbi": {"dailyUV": tb_daily},
        "huanbi": {"dailyUV": hb_daily},
        "source": "sls",
    })
