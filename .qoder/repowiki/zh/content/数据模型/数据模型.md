...
## 更新摘要
**变更内容**   
- 增强了数据库迁移机制，支持更复杂的字段演进和类型转换
- 新增平台耗时监控字段 platform_elapsed，用于性能分析
- 新增记录类型字段 record_type，支持周记录和月记录的区分
- 增强学校管理功能，新增 owner_id、display_name、type、priority 等字段
- 实现基于角色的访问控制，支持 super_admin/admin/user 三级权限体系
- 完善用户模型，新增 display_name 字段和角色属性方法
...
- [services/collector.py:780-862](file://services/collector.py#L780-L862)
...
## 核心组件
- **增强的连接与迁移**：统一通过上下文管理器获取连接，启用 WAL 与外键约束，并在每次连接时执行增量迁移与列添加逻辑，确保版本演进兼容。支持复杂的数据类型迁移和字段演进。
- **角色化实体模型**：使用 dataclass 定义轻量对象，封装保存、删除、查询等方法，内部直接调用 get_connection 执行 SQL。用户模型支持三级角色权限控制。
- **时间序列记录**：周/月记录以"学校+年份+周期标识"为唯一键，采用 UPSERT 语义避免重复写入并支持幂等更新。新增 platform_elapsed 字段用于性能监控。
- **数据来源标记**：记录包含 data_source 字段，区分 grafana 与 database 两种采集路径，支持 record_type 字段区分任务类型。
...
- [models/user.py:43-49](file://models/user.py#L43-L49)
...
**图表来源**
- [web/routes/school.py:69-112](file://web/routes/school.py#L69-L112)
- [web/routes/user.py:71-99](file://web/routes/user.py#L71-L99)
- [models/school.py:29-78](file://models/school.py#L29-L78)
...
- **高级增量迁移**：
  - 首次启动时 init_db 会创建所有表（若不存在），并对已有表执行"缺失列添加"的增量迁移。
...
AddCol --> |否| Proceed["继续"]
Alter --> Proceed
Proceed --> InitDB["init_db() 执行完整迁移"]
InitDB --> MigrateTypes["_migrate_if_needed() 类型迁移"]
MigrateTypes --> SeedData["_seed_users_from_excel() 种子数据"]
SeedData --> YieldConn["yield conn(事务上下文)"]
...
- **schools**：name 唯一；包含多平台名称映射、学段/年级/学部/校区、排序、创建者、显示名、类型、优先级等。
- **users**：username 唯一；包含密码与各平台凭证、分配学校、管理员标志、角色、真实姓名、时间戳。
- **collect_tasks**：任务调度与汇总信息，新增 record_type 字段区分任务类型。
...
- **字段与约束**：name 唯一；display_name 可缺省但保存时回退为 name；type 用于区分直营/托管校；owner_id 记录创建者；priority 控制优先级（高/中/低）；sort_order 控制排序。
- **CRUD 行为**：
...
**章节来源**
- [models/school.py:29-78](file://models/school.py#L29-L78)
- [models/school.py:85-170](file://models/school.py#L85-L170)
- [web/routes/school.py:69-112](file://web/routes/school.py#L69-L112)

### 用户（User）模型：安全设计与权限控制
- **字段与约束**：username 唯一；password 明文存储（当前实现未哈希）；is_admin 布尔型；role 字符串型（super_admin/admin/user）；assigned_schools 文本，逗号/中文分隔；display_name 真实姓名。
- **认证与权限**：
...
**章节来源**
- [models/user.py:51-105](file://models/user.py#L51-L105)
- [models/user.py:107-126](file://models/user.py#L107-L126)
- [web/routes/user.py:71-99](file://web/routes/user.py#L71-L99)
...
- **任务类型支持**：支持 record_type 参数区分周记录和月记录采集任务。
...
- **服务层依赖**：依赖模型层进行数据持久化，屏蔽底层 SQL 细节。
- **Web 路由层**：对模型进行薄封装，负责鉴权与输入校验。
...
## 结论
数据模型层以统一的连接与迁移机制为基础，围绕学校、用户、周/月记录等核心实体构建了清晰的 CRUD 与查询能力。通过唯一键与 UPSERT 实现幂等写入，结合数据来源标记与平台耗时记录，便于追踪与排障。新增的角色化权限控制和学校管理功能显著提升了系统的可维护性和安全性。建议在安全性（密码哈希、凭证加密）、索引优化、批量写入与数据归档方面持续改进，以提升系统稳定性与性能。
