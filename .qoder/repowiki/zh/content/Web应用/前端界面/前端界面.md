...
## 更新摘要
**变更内容**   
- 新增搜索输入同步功能，实现输入框与隐藏select的双向数据同步
- 增强偏好设置恢复机制，支持用户级别的筛选条件持久化
- 新增PNG导出按钮，支持图表截图导出功能
- 优化用户体验，提升界面交互流畅度
...
SCHOOL_DETAIL["web/templates/school_detail.html<br/>学校详情页"]
end
subgraph "静态资源"
...
- 页面模板：index.html（仪表盘）、collect.html（数据采集）、charts.html（图表分析）、users.html（用户管理）、school_detail.html（学校详情）。
...
### 可搜索下拉组件与搜索输入同步
**新增功能** 实现了智能的可搜索下拉组件，支持输入框与隐藏select的双向数据同步。

- 组件特性
  - 支持键盘导航（上下箭头选择、Enter确认、Escape关闭）
  - 实时过滤选项列表
  - 输入框清空时自动同步清空隐藏select值
  - 失焦时保持数据一致性
  - 支持分组数据显示
...
sequenceDiagram
participant U as "用户"
participant SS as "可搜索下拉组件"
participant SEL as "隐藏select元素"
U->>SS : 输入搜索文本
SS->>SS : 过滤选项列表
SS->>SEL : 同步清空空值
U->>SS : 点击选项
...
**图示来源**
- [web/templates/index.html:803-843](file://web/templates/index.html#L803-L843)
- [web/templates/school_detail.html:281-396](file://web/templates/school_detail.html#L281-L396)
...
### 偏好设置恢复机制
**新增功能** 实现了用户级别的筛选偏好记忆和恢复功能，提升用户体验连续性。

- 功能特性
  - 按用户ID隔离存储偏好设置
  - 支持多种筛选条件的持久化（学校、学段、年级、学科、日期范围）
  - 页面加载时自动恢复上次使用的筛选条件
  - 支持学校类型Tab状态的保存与恢复

```mermaid
flowchart TD
Init["页面初始化"] --> CheckPrefs["检查localStorage偏好"]
CheckPrefs --> HasPrefs{"是否有保存的偏好?"}
HasPrefs --> |是| Restore["恢复筛选条件"]
HasPrefs --> |否| Default["使用默认值"]
Restore --> Query["执行查询"]
Default --> Query
Query --> SaveOnAction["用户操作时保存偏好"]
SaveOnAction --> LocalStorage["保存到localStorage"]
```

**图示来源**
- [web/templates/index.html:845-888](file://web/templates/index.html#L845-L888)
- [web/templates/school_detail.html:506-550](file://web/templates/school_detail.html#L506-L550)
...
- 交互逻辑
  ...
  - **新增** restorePreferences 恢复用户筛选偏好
...
- [web/routes/charts.py:70-120](file://web/routes/charts.py#L70-120)
- [web/routes/charts.py:323-347](file://web/routes/charts.py#L323-347)
...
### PNG导出功能
**新增功能** 实现了图表PNG导出功能，支持将当前页面的图表内容导出为高质量图片。
...
sequenceDiagram
participant U as "用户"
participant EXP as "导出功能"
participant H2C as "html2canvas库"
U->>EXP : 点击"导出PNG"按钮
EXP->>EXP : 临时隐藏操作按钮
EXP->>H2C : 调用html2canvas()
H2C-->>EXP : 返回Canvas对象
EXP->>EXP : 恢复操作按钮显示
EXP->>EXP : 生成下载链接
EXP-->>U : 触发PNG文件下载
...
**图示来源**
- [web/templates/base.html:329-370](file://web/templates/base.html#L329-L370)
...
- 外部依赖
  ...
  - school_detail.html 通过 CDN 引入 html2canvas 库用于PNG导出。
...
MAIN --> TPL_SCHOOL["templates/school_detail.html"]
...
SCHOOL_TPL["templates/school_detail.html"] --> CDN3["CDN html2canvas"]
```
...
- **新增** html2canvas 库通过 CDN 引入，支持PNG导出功能。
- 建议
...
- **新增** html2canvas 库在不同浏览器下的兼容性测试。
- 建议
  - 对不支持的特性提供回退样式与行为。
  - 在目标浏览器矩阵下测试关键交互（SSE、fetch、EventSource、PNG导出）。
...
- **新增** 搜索输入同步：输入框与隐藏select双向数据同步，提升表单一致性。
- **新增** 偏好设置恢复：用户筛选条件持久化，提升操作连续性。
- **新增** PNG导出功能：一键导出图表为高清图片，支持报告分享。
- 建议
...
- **新增** PNG导出问题
  - 若html2canvas库加载失败，会显示提示信息。
  - 某些跨域资源可能导致截图不完整，需要配置useCORS选项。
- **新增** 偏好设置恢复问题
  - localStorage容量限制可能导致偏好设置丢失。
  - 用户清除浏览器数据后会失去偏好设置。
...
- [web/routes/charts.py:323-347](file://web/routes/charts.py#L323-347)
- [web/templates/base.html:345-350](file://web/templates/base.html#L345-L350)

## 结论
该前端界面系统采用简洁的模板继承与全局样式/脚本组织方式，配合 Flask 蓝图提供清晰的页面与 API 边界。通过 CSS 变量实现主题定制，利用 SSE 实现采集进度实时反馈，整体具备良好的可维护性与扩展性。

**最新更新** 系统增强了用户体验，包括搜索输入同步、偏好设置恢复和PNG导出等功能，进一步提升了界面的易用性和功能性。后续可在组件化、模块化与兼容性方面持续优化，以提升性能与用户体验。